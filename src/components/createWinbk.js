        ...
        if(url.indexOf('?') != -1){
          ...
          .then(res => {
            ...
            // console.log(res.data.IMAGES);
                ...
                var LOBValue= Object.keys(other_filters.LOB[0])[0];
                var ImperativeValue= Object.keys(other_filters.Imperative[0])[0];
                this.setState({
                    LOB: LOBValue,
                    Imperative: ImperativeValue,
                    SolutionArea: res.data.SOLUTION_AREAS[0].FILTER_ID,
                ...
                setTimeout(()=>{ 
                    ...
                    if(res.data.WINSTORY_MAPPED_L2_FILTERS!=""){
                    ...
                    if(other_filters.solution_use_case){
                        var selectedElement = document.getElementById('SolutionUsecase');
                            ...
                            solusecase.push(other_filters.solution_use_case[p]);   
                            }
                    ...
                    if(other_filters.LOB){
                        ...
                        var lobsecondlevel = []
                            ...
                            lobsecondlevel.push(other_filters.LOB[p][LOBValue]);   
                            }
                    ...
                    if(other_filters.Imperative){
                        ...
                        var imperativesecondlevel = []
                           for(var p=0;p<=other_filters.Imperative.length-1;p++){
                            imperativesecondlevel.push(other_filters.Imperative[p][ImperativeValue]);   
                           }
                    ...
                    //     var selectedElement2 = document.getElementById('LOBSecondLevel');

                    //     var LOBType = Object.keys(other_filters.LOB[0])[0] 
                    //     var lobs = []
                    //     for(var a=0;a<=other_filters.LOB.length-1;a++){
                    //         console.log(Object.keys(other_filters.LOB[a]))
                    //         lobs.push(Object.keys(other_filters.LOB[a]));   
                    //     }
                    //     console.log(lobs)
                    //     var values = lobsecondlevel;
                    //     for (var selectSol = 0; selectSol < selectedElement2.options.length; selectSol++) {
                    //         selectedElement2.options[selectSol].selected = values1.indexOf(selectedElement2.options[selectSol].value) >= 0;
                    //     }

                    //     var lobsecondlevel = []
                    //     console.log(LOBType)
                    //     for(var p=0;p<=other_filters.LOB.length-1;p++){
                    //         console.log(other_filters.LOB[p], other_filters.LOB[p][LOBType])
                    //         lobsecondlevel.push(other_filters.LOB[p][LOBType]);   
                    //     }
                    //     console.log(lobsecondlevel)
                    //     var values1 = lobsecondlevel;
                    //     for (var selectSol = 0; selectSol < selectedElement2.options.length; selectSol++) {
                    //         selectedElement2.options[selectSol].selected = values1.indexOf(selectedElement2.options[selectSol].value) >= 0;
                    //     }
            ...
            if(res.data.WINSTORY_THUMBNAIL!=""){
                let img = new Image();
                img.src = res.data.WINSTORY_THUMBNAIL ;
            ...
            .then(res => {
                ...
                for (var i = 0; i < res.data.allFilters.length; i++) {
                    ...
                    if (FilterData[i].Type == "Solution Area") {
                        ...
                        //     global.SolutionAreaTypeL2.push({
                        //         "FILTER_ID" : FilterData[i].filters[Solfilter].FILTER_ID,
                        //         "FILTER_TYPE_L2" :FilterData[i].filters[Solfilter].FILTER_TYPE_L2
                        //         })
                        // }
                        this.setState({
                        ...
                        if(this.state.id==""){
                            this.setState({
                                SolutionArea: this.state.solutionFilterdrop[0].FILTER_ID
                            })
                        }
                        ...
                        for (var AssetTypefilter = 0; AssetTypefilter < FilterData[i].filters.length; AssetTypefilter++) {
                            if(FilterData[i].filters[AssetTypefilter].FILTER_NAME=="Win Stories"){
                                this.setState({
...
if((e.target.getAttribute("id")=="notionalArchtechure")){
    ...
    onCreateClick = (e) => {
        ...
        var Solution_use_case_array = [];
            ...
            l1_array.push(this.state.SolutionArea)
            ...
            for (r = 0; r < dropDownLOB.options.length ; r += 1) {
                if (dropDownLOB.options[r].selected) {
                    //countryArray.push( dropDown.options[i].value); //If you need only values 
                    LOB_array.push({
                        [this.state.LOB]: dropDownLOB.options[r].value
                    });
                }
            }
            ...
            for (t = 0; t < dropDownSolutionUseCase.options.length ; t += 1) {
                if (dropDownSolutionUseCase.options[t].selected) {
                    //countryArray.push( dropDown.options[i].value); //If you need only values 
                    Solution_use_case_array.push(dropDownSolutionUseCase.options[t].value);
                }
            }
            for (u = 0; u < dropDownImperative.options.length ; u += 1) {
                if (dropDownImperative.options[u].selected) {
                    //countryArray.push( dropDown.options[i].value); //If you need only values 
                    Imperative_array.push({
                        [this.state.Imperative]:dropDownImperative.options[u].value
                    });
                }
            }
        ...
        if(document.getElementById('WinStoryId').value==""){ 
            ...
            //         alert(`Please select all the ${this.state.SolutionArea} Areas`);
            //         document.getElementById('SolutionArea').focus();
            //         return false;
            //         e.preventDefault();
            //     }
         ...
         <Row>
            ...
            <Form id="createForm">
               ...
               <Row>
                ...
                <Form.Group as={Col}  md={6} >
                     <Form.Label>Deal Size($ARR)  <sup>*</sup></Form.Label>
               ...
               <Row>
                  ...
                  <Form.Group as={Col}  md={6} >
                     <Form.Label>Solution Area <sup></sup><span title="Select all solution areas this asset involves ">&#x1f6c8;</span></Form.Label>
                      <div  class="inlineDiv">
                      <Form.Control as="select"  id="SolutionArea" name='SolutionArea' onChange={e => this.handleChangeSecondLevel(e)} value={this.state.SolutionArea}> 
                      {
               ...
               <Form.Group as={Col}  md={6} >
                ...
                <Form.Group as={Col}  md={6} >
                     <Form.Label>Renewal <sup>*</sup></Form.Label>
               ...
               <Row>
                ...
                <Form.Group as={Col}  md={6} >
                <Form.Label>Imperative <sup></sup></Form.Label>
                     <br/>
                     <div class="inlineDiv">
               ...
               <Row>
                  <Form.Group as={Col}  md={12} >
                     <Form.Label>Why the Customer Bought/ The Imact of the purchase to the customer <sup>*</sup><span title="250 characters max.">&#x1f6c8;</span></Form.Label>
               ...
               <Row>
                  <Form.Group as={Col}  md={12} >
                     <Form.Label>Lessons Learnt<sup>*</sup><span title="250 characters max.">&#x1f6c8;</span></Form.Label>
