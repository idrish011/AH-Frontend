...
export class ImageCard extends Component {
    ...
    componentDidMount() {
        ...
        if ((sessionStorage.getItem('user_email') != "") && (sessionStorage.getItem('user_email') != null) && (sessionStorage.getItem('user_email') != undefined)) {
            // sessionStorage.setItem("role", "winloader");
            axios.get(global.Ip + global.Port + '/winstory/mywinstory', {
                headers: {
                    "user_email": sessionStorage.getItem("user_email"), "user_role": sessionStorage.getItem("role")
                }
            })
                .then(res => {
                    ...
                    for (var i = 0; i < res.data.length; i++) {
                        ...
                        if (res.data[i].status == "Live") {
                            ...
                            // if (typeof WinStoryDataLive.GROUP_TYPE != "undefined") {
                            //     alert("GOT THERE");
                            //  }
    ...
    render() {
        ...
        if (WinStoryDataLivenew.length > 0) {
            LiveCnt = "Live Wins (" + WinStoryDataLivenew.length + ")";
        } else {
            LiveCnt = "Live Wins";
        }
        var UnderReviewCnt;
        ...
        return (
            <Row className="sectionCover myWin" id="dataAssetShow">
                        ...
                        <Tab eventKey="Live" title={LiveCnt}>
                            {WinStoryDataLivenew.length > 0 && <>
                                {WinStoryDataLivenew.map((WinStoryDataLive, index) =>
                                    <Col md={9} data-id={WinStoryDataLive.WINSTORY_ID} className="mb-4 ">
                                        <div class="clearfix bord-1">
                                            <Row>
                                                <Col md={10}>
                                                    {/* <Image src={WinStoryDataLive.WINSTORY_LOGO} /> */}
                                                    <img src={WinStoryDataLive.WINSTORY_LOGO+'?'+new Date().getMilliseconds()} />
                                                </Col>
                                                <Col md={2}>
                                                    {/* {WinStoryDataLive.GROUP_TYPE[0].FILTER_NAME} */}
                                                </Col>
                                            </Row>
