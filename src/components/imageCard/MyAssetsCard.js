...
export class ImageCard extends Component {
    state = {
        AssetDataLive: [],AssetUnderReview: [],PendingRectification:[],AssetRejected: [], AssetSaved:[],
            ...
            //   console.log(res.data.length);
              for(var i=0;i<res.data.length;i++){
                ...
                //    }
                    ...
                    if(res.data[i].status=="Saved"){
                        for(var j=0;j<res.data[i].arr.length;j++){
                            if(res.data[i].arr[j].ASSET_REVIEW_NOTE==null){
                                res.data[i].arr[j].ASSET_REVIEW_NOTE=[];
                           }
                          }
                        const AssetSaved = res.data[i].arr;
                      //  global.AssetCount=AssetData;
                        this.setState({ AssetSaved });
  ...
  render() {
    ...
    //   }else{
        ...
        AssetSavednew=this.state.AssetSaved;
        ...
        var AssetSavednewCnt;
        if(AssetSavednew.length>0){
            AssetSavednewCnt="Saved ("+AssetSavednew.length+")";
        }else{
            AssetSavednewCnt="Saved";
        }
        ...
        <Row className="sectionCover myAsset" id="dataAssetShow">
           ...
           <Col className="col-md-12">
                        ...
                        <Tab eventKey="UnderReview" title={UnderReviewCnt}>
                                ...
                                <Tab eventKey={AssetSavednewCnt} title={AssetSavednewCnt}>
                                {AssetSavednew.length>0 && <>
                                {AssetSavednew.map((AssetSaved,index)=> 
                                    <Col md={9} data-id={AssetSaved.ASSET_ID} className="mb-4 ">
                                        <div class="clearfix bord-1"><Image src={AssetSaved.ASSET_THUMBNAIL} rounded  />
                                            <h5>{AssetSaved.ASSET_TITLE}</h5>
                                            <span className="descRightPanel"> <ReactMarkdown source={AssetSaved.ASSET_DESCRIPTION} />  </span>

                                            {/* <p>{AssetRejected.ASSET_DESCRIPTION}</p> */}
                                            <span class="status underReview">&#9679; {AssetSaved.ASSET_STATUS}</span>
                                            <div className="small mt-20"><Moment format="DD MMMM YYYY">{AssetSaved.ASSET_CREATED_DATE}</Moment></div>
                                            {/* {AssetRejected.ASSET_REVIEW_NOTE !=null && */}
                                            <>
                                            <div className="Notes hide" id={"NotesVal"+AssetSaved.ASSET_ID}> {AssetSaved.ASSET_REVIEW_NOTE.map(ASSET_REVIEW_NOTE => 
                                            <>{ASSET_REVIEW_NOTE.note +'Title' + ASSET_REVIEW_NOTE.section+'next'}</>
                                            )}
                                            </div>
                                           
                                            <div classname="mt-20"><span className="btn btn-primary" onClick={this.handleViewNote(AssetSaved.ASSET_ID)}>VIEW </span>
                                            &nbsp;&nbsp;<a className="btn btn-primary" href="javscript:void(0);" onClick={this.handleEdit(AssetSaved.ASSET_ID)}>EDIT</a>
                                            </div>
                                            </>
                                            {/* } */}
                                        </div>
                                 ...
                                 {AssetSavednew.length<=0 && <p class="text-center">
                                     No Records Yet.
                                </p>}
                                </Tab>
