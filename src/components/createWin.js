...
class AssetDetails extends Component {
    ...
    componentDidMount() {
        ...
        if (url.indexOf('?') != -1) {
            ...
            axios.get(global.Ip + global.Port + '/winstory/' + ID[1], {
                ...
                setTimeout(() => {
                    ...
                    var arrSol = []; var arrIndus = []; var arrCometitor = []; var arrPfrm = []; var arrAsst = []; var arrWin = []; var arrLob = [];
                    for (var valuesol = 0; valuesol < res.data.FILTERMAP.length; valuesol++) {
                        ...
                        if (res.data.FILTERMAP[valuesol].TYPE == 'LOB') {
                            var selectedElement = document.getElementById('LOB');

                            // alert('1'+res.data.FILTERMAP[valuesol]);
                            for (var p = 0; p <= res.data.FILTERMAP[valuesol].arr.length - 1; p++) {
                                arrLob.push(res.data.FILTERMAP[valuesol].arr[p].FILTER_ID);
                            }
                            var values2 = arrLob;
                            for (var selectSol = 0; selectSol < selectedElement.options.length; selectSol++) {
                                selectedElement.options[selectSol].selected = values2.indexOf(selectedElement.options[selectSol].value) >= 0;
             ...
             //   var LOBValue = "";  //[]
                ...
                if((res.data.WINSTORY_CREATED_ON!="")  && (res.data.WINSTORY_CREATED_ON!=null)&& (res.data.WINSTORY_CREATED_ON!="null")){
                    var SplitPicked = res.data.WINSTORY_CREATED_ON.split("T");
                    var SplitPicked1=SplitPicked[0];

                }else{
                    var SplitPicked1="";

                }
                if((res.data.WINSTORY_PROPOSED_GOLIVE_DATE!="")  && (res.data.WINSTORY_PROPOSED_GOLIVE_DATE!=null)&&(res.data.WINSTORY_PROPOSED_GOLIVE_DATE!="null")){
                    var SplitPropPicked = res.data.WINSTORY_PROPOSED_GOLIVE_DATE.split("T");
                    var SplitPropPicked1=SplitPropPicked[0];
                }else{
                    var SplitPropPicked1="";

                }
                if((res.data.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET!="")  && (res.data.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET!=null)&& (res.data.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET!="null")){
                    var SplitTarPicked = res.data.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET.split("T");
                    var  SplitTarPicked1=SplitTarPicked[0];
                }else{
                    var SplitTarPicked1="";
                ...
                this.setState({
                    ...
                    WINSTORY_REFERENCE_ASSET:res.data.WINSTORY_REFERENCE_ASSET,
                    WINSTORY_NOTES:res.data.WINSTORY_NOTES,
                    WINSTORY_REFERENCE_PLAN: res.data.WINSTORY_REFERENCE_PLAN,
                    WINSTORY_CUSTOMER_CONTACT: res.data.WINSTORY_CUSTOMER_CONTACT,
                    ECAEnggLead:res.data.WINSTORY_ECA_ENGG_LEAD,

                    Saleslead: res.data.WINSTORY_SALES_LEAD,
                    WinStoryOwner: res.data.WINSTORY_CREATED_BY,
                    WinFiscalQuarter: res.data.WINSTORY_FISCAL_QUARTER,
                    WINSTORY_CUSTOMER_LIFECYCLE:res.data.WINSTORY_CUSTOMER_LIFECYCLE,
                    WINSTORY_REFERENCE_LANGUAGE_INCLUDED:res.data.WINSTORY_REFERENCE_LANGUAGE_INCLUDED,
                    WINSTORY_REFERENCE_STATUS:res.data.WINSTORY_REFERENCE_STATUS,
                    ...
                    arch: arch,
                    createdDate: SplitPicked1,
                    WINSTORY_PROPOSED_GOLIVE_DATE:SplitPropPicked1,
                    WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET:SplitTarPicked1,
                    ...
                    RegistryID:res.data.WINSTORY_REG_ID,
                    SubscriptionID:res.data.WINSTORY_SPN,
                    consumingStatus:res.data.WINSTORY_REFERENCEABLE,
                    // "WINSTORY_CONSUMING":this.state.consumingStatus
        ...
        axios.get(global.Ip + global.Port + '/asset/allfilters', {
            ...
            for (var i = 0; i < res.data.allFilters.length; i++) {
                ...
                else if (FilterData[i].Type == "LOB") {
                    var LOBListingDrop = " ";
                    for (var LOBListingfilter = 0; LOBListingfilter < FilterData[i].filters.length; LOBListingfilter++) {
                        LOBListingDrop += "<option value='" + FilterData[i].filters[LOBListingfilter].FILTER_ID + "'>" + FilterData[i].filters[LOBListingfilter].FILTER_NAME + '</option>';
                    }
                    this.setState({
                        LOBListingArr: LOBListingDrop
                    })
                }
    ...
    handleChange = (e) => {
        ...
        // if ((e.target.name == "createdDate")||(e.target.name="WINSTORY_PROPOSED_GOLIVE_DATE")||(e.target.name="WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET")) {
   ...
   //     alert(e);
    ...
    onCreateClick = (e) => {
        ...
        var dropDownLOB = document.getElementById('LOB'), r;
        ...
        for (n = 0; n < dropDownLOB.options.length; n += 1) {
            if (dropDownLOB.options[n].selected) {
                //countryArray.push( dropDown.options[i].value); //If you need only values 
                l1_array.push(dropDownLOB.options[n].value);
            }
        }
        ...
        if (document.getElementById('WinStoryId').value == "") {
            if (this.state.Industry == "") {
                ...
                alert('Please enter Asset Cloud Services');
        ...
        var selectedPropdate = document.getElementById('WINSTORY_PROPOSED_GOLIVE_DATE').value;
        
        //  alert(selecteddate);
        if (selectedPropdate != "") {
            var splitPropDate = selectedPropdate.split("-");
            var month = new Array();
            month[0] = "January";
            month[1] = "February";
            month[2] = "March";
            ...
            // const month = date.toLocaleString('default', { month: 'long' });
            // console.log(month +'20');
            console.log(splitPropDate);
            if (splitPropDate[1] < 10) {
                var splitProp = splitPropDate[1].split("0");
                var monthAppndProp = splitProp[1] - 1;
            } else {
                var monthAppndProp = splitPropDate[1] - 1;

            }
            //  console.log(month[monthAppnd]);
            // alert(month[monthAppnd]);
            var dtProp = splitPropDate[2] + "-" + month[monthAppndProp] + "-" + splitPropDate[0];
        }

        var selectedTardate = document.getElementById('WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET').value;
        
        //  alert(selecteddate);
        if (selectedPropdate != "") {
            var splitTarDate = selectedTardate.split("-");
            var month = new Array();
            month[0] = "January";
            month[1] = "February";
            month[2] = "March";
            ...
            // const month = date.toLocaleString('default', { month: 'long' });
            // console.log(month +'20');
            console.log(splitTarDate);
            if (splitPropDate[1] < 10) {
                var splitTar = splitTarDate[1].split("0");
                var monthAppndTar = splitTar[1] - 1;
            } else {
                var monthAppndTar = splitTarDate[1] - 1;

            }
            //  console.log(month[monthAppnd]);
            // alert(month[monthAppnd]);
            var dtTar = splitPropDate[2] + "-" + month[monthAppndProp] + "-" + splitPropDate[0];
        }
// alert(this.state.RegistryID);
// alert(this.state.SubscriptionID);
        //    this.setState({
        //        createdDate: dt
        //    }) 
        var reqParms = {
            ...
            "WINSTORY_NOTES":this.state.WINSTORY_NOTES,
            "WINSTORY_REFERENCE_ASSET":this.state.WINSTORY_REFERENCE_ASSET,
            "WINSTORY_REFERENCE_PLAN": this.state.WINSTORY_REFERENCE_PLAN,
            "WINSTORY_CUSTOMER_CONTACT":this.state.WINSTORY_CUSTOMER_CONTACT,
            "WINSTORY_SALES_LEAD":this.state.Saleslead,
            "WINSTORY_ECA_ENGG_LEAD":this.state.ECAEnggLead,
            ...
            "WINSTORY_CUSTOMER_LIFECYCLE":this.state.WINSTORY_CUSTOMER_LIFECYCLE,
            "WINSTORY_REFERENCE_LANGUAGE_INCLUDED":this.state.WINSTORY_REFERENCE_LANGUAGE_INCLUDED,
            "WINSTORY_REFERENCE_STATUS":this.state.WINSTORY_REFERENCE_STATUS,
            ...
            "WINSTORY_PROPOSED_GOLIVE_DATE":dtProp,
            "WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET":dtTar,
            "created_on": dt,
            "user_email": sessionStorage.getItem("user_email"),
            ...
            "WINSTORY_REG_ID":this.state.RegistryID,
            "WINSTORY_SPN":this.state.SubscriptionID,
            "WINSTORY_REFERENCEABLE":this.state.consumingStatus
            ...
            axios.post(url, reqParms, {
                ...
                .then(response => {
                    ...
                    if (response.data.status != 'failed') {
                        alert('Your win story has been created. Kindly check in "My Wins", for further updates.');
            ...
            axios.put(url, reqParms, {
                ...
                .then(response => {
                    ...
                    if (response.data.status != 'failed') {
                        ...
                        alert('Your win story has been updated. Kindly check in "My Wins" for further updates.');
                        // window.location.href = "/MyAssets";
                        this.setState({
                            LoginSuccessRedirect: true
                        });
    ...
    render() {
        ...
        if (this.state.WINSTORY_PROPOSED_GOLIVE_DATE != "") {
            var propDate = this.state.WINSTORY_PROPOSED_GOLIVE_DATE;
        } else {
            var nowDate = new Date();
            var propDate = nowDate.getFullYear() + "-" + (((nowDate.getMonth() + 1) < 10) ? "0" : "") + (nowDate.getMonth() + 1) + "-" + ((nowDate.getDate() < 10) ? "0" : "") + nowDate.getDate();
            // alert(todayDate);
        }
        

        if (this.state.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET != "") {
            var tarDate = this.state.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET;
        } else {
            var nowDate = new Date();
            var tarDate = nowDate.getFullYear() + "-" + (((nowDate.getMonth() + 1) < 10) ? "0" : "") + (nowDate.getMonth() + 1) + "-" + ((nowDate.getDate() < 10) ? "0" : "") + nowDate.getDate();
            // alert(todayDate);
        }
                        ...
                        - Development Center, not DC">&#x1f6c8;</span></Form.Label>
                                        ...
                                        <Row>
                                            <Form.Group as={Col} md={6} >
                                                ...
                                                <Form.Control as="textarea" id="WinStoryOwner" name='WinStoryOwner' maxLength="500"
                                            ...
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>Referenceable Account?</Form.Label>
                                                <br/>
                                                {/* <label class="switch">
                                                ...
                                                {/* {sessionStorage.getItem('leader') == "true" && */}
                                                    <span class=""><label class="switch">
                                                        <input type="checkbox" id="togBtn" value={this.state.consumingStatus} name="disablePromote" checked={this.state.consumingStatus == "yes" ? "checked" : ""} onChange={e => this.ConsumingWin(this.state.consumingStatus)}/>
                                                        <span class="slider round"></span>
                                                    </label></span>
                                                {/* } */}
                                                    ...
                                                    {/* <input type="checkbox" name="consumingAccount" id="consumingAccount"/><br/> */}
                                                {/* <small>Hint: Only Email ID is acceptable. Please use ',' separator in case of multiple Email IDs </small> */}
                                            </Form.Group>
                                        </Row>
                                        <Row>
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>Sales Lead<span title="Sales lead"></span></Form.Label>
                                                <Form.Control as="textarea" id="Saleslead" maxlength="250" name='Saleslead'
                                                    value={this.state.Saleslead}
                                                    onChange={e => this.handleChange(e)} placeholder="Enter Sales Lead" /><br />
                                                <small>Hint: Only Email ID is acceptable. Please use ',' separator in case of multiple Email IDs </small>
                                            </Form.Group>
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>Reference Status</Form.Label>
                                                <Form.Control as="select" id="WINSTORY_REFERENCE_STATUS" name='WINSTORY_REFERENCE_STATUS' onChange={e => this.handleChange(e)} value={this.state.WINSTORY_REFERENCE_STATUS}>
                                                    <option value="Prospective">Prospective</option>
                                                    <option value="Engaged">Engaged</option>
                                                    <option value="Referenceable">Referenceable</option>
                                                </Form.Control>
                                            </Form.Group>
                                        ...
                                        <Row>
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>Account Team<sup>*</sup><span title="Email address and 250 characters max.">&#x1f6c8;</span></Form.Label>
                                                <Form.Control as="textarea" id="RepAndSE" name='RepAndSE' maxLength="250"
                                                    value={this.state.RepAndSE}
                                                    onChange={e => this.handleChange(e)} placeholder="Enter Account Team" >
                                            ...
                                            <Form.Group as={Col} md={6} >
                                            <Form.Label>ECA/Engineer Lead<span title="ECA/Engineer Lead"></span></Form.Label>
                                                <Form.Control as="textarea" id="ECAEnggLead" name='ECAEnggLead'
                                                    value={this.state.ECAEnggLead}
                                                    onChange={e => this.handleChange(e)} maxlength="250" placeholder="ECA/Engineer Lead" /><br />
                                                 <small>Hint: Only Email ID is acceptable. Please use ',' separator in case of multiple Email IDs </small>
                                        ...
                                        <Row>
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>Cloud Services <sup>*</sup> <span title="All the different types of collateral that belong to this asset should be selected. this asset should be selected">&#x1f6c8;</span></Form.Label>
                        ...
                        return <option value={solArea.FILTER_ID}>{solArea.FILTER_NAME}</option>
                                        ...
                                        <Row>
                                            <Form.Group as={Col} md={6} >
                                                ...
                                                <br />
                                                <div class="inlineDiv">
                                                <Form.Control as="select" multiple id="LOB" name='LOB' onChange={e => this.handleChange(e)} dangerouslySetInnerHTML={{ __html: this.state.LOBListingArr }}>
                                                </Form.Control>
                        ...
                        <Form.Control as="select" multiple id="AssetType" name='AssetType' onChange={e => this.handleChange(e)} dangerouslySetInnerHTML={{ __html: this.state.AssetTypeID }}> 
                                        ...
                                        <Row class="mt-20 " >
                                            <Form.Group className="col-md-6">
                                                <Form.Label>Registry ID <span title="Upload the default image that would be displayed as Logo for the Win.">&#x1f6c8;</span></Form.Label>
                                                {/* <Form.Label>Account Team<sup>*</sup><span title="Email address">&#x1f6c8;</span></Form.Label> */}
                                                <Form.Control type="text" id="RegistryID" name='RegistryID'
                                                    value={this.state.RegistryID}
                                                    onChange={e => this.handleChange(e)} placeholder="Registry ID" /><br />
                                                {/* <small>hint: emailid is acceptable only. Please use ',' to separate multiple email ids </small> */}
                                            ...
                                            {/* <div class="mt-20 "> */}
                                                <Form.Group className="col-md-6">
                                                    <Form.Label>Subscription Plan Number  </Form.Label>
                                                    {/* <Form.Label>Account Team<sup>*</sup><span title="Email address">&#x1f6c8;</span></Form.Label> */}
                                                <Form.Control type="text" id="SubscriptionID" name='SubscriptionID'
                                                    value={this.state.SubscriptionID}
                                                    onChange={e => this.handleChange(e)} placeholder="Subscription Plan Number" /><br />
                                                {/* <small>hint: emailid is acceptable only. Please use ',' to separate multiple email ids </small> */}
                                                </Form.Group>
                                            {/* </div> */}
                                        ...
                                        <Row>
                                            ...
                                            <Form.Group as={Col} md={6} >
                                                    <Form.Label>Customer Lifecyle </Form.Label>
                                                    <br />
                                                    {/* {this.state.WinFiscalQuarter + "Months"}  */}
                                                    <Form.Control as="select" id="WINSTORY_CUSTOMER_LIFECYCLE" name='WINSTORY_CUSTOMER_LIFECYCLE' onChange={e => this.handleChange(e)} value={this.state.WINSTORY_CUSTOMER_LIFECYCLE}>
                                                        <option value="New Win">New Win</option>
                                                ...
                                                <Form.Group as={Col} md={6} >
                                                    <Form.Label>Was reference lanuage included in the contract?</Form.Label>
                                                    <Form.Control as="select" id="WINSTORY_REFERENCE_LANGUAGE_INCLUDED" name='WINSTORY_REFERENCE_LANGUAGE_INCLUDED' onChange={e => this.handleChange(e)} value={this.state.WINSTORY_REFERENCE_LANGUAGE_INCLUDED}>
                                        ...
                                        <Row>
                                        <Form.Group as={Col} md={6} >
                                                <Form.Label>What is the proposed Go-Live date?</Form.Label>
                                                <br />
                                               
                                                    <Form.Control id="WINSTORY_PROPOSED_GOLIVE_DATE" name='WINSTORY_PROPOSED_GOLIVE_DATE' type="date" placeholder="Created Date"
                                                        onChange={e => this.handleChange(e)} 
                                                        value={this.state.WINSTORY_PROPOSED_GOLIVE_DATE}
                                                        defaultValue={propDate}
                                                    />
                                                     
                                            </Form.Group>
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>What is the first consumption milestone usage target date?</Form.Label>
                                                <Form.Control id="WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET" name='WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET' type="date" placeholder="Created Date"
                                                        onChange={e => this.handleChange(e)}
                                                         value={this.state.WINSTORY_CONSUMPTION_MILESTONE_USAGE_TARGET}
                                                        defaultValue={tarDate}
                                                    />
                                            </Form.Group>
                                        ...
                                        <Row>
                                            <Form.Group as={Col} md={6} >
                                                ...
                                                <Form.Control type="text" id="WINSTORY_CUSTOMER_CONTACT" name='WINSTORY_CUSTOMER_CONTACT'
                                                    value={this.state.WINSTORY_CUSTOMER_CONTACT}
                                                    onChange={e => this.handleChange(e)} placeholder="Enter Customer Contact" /><br />
                                                
                                            </Form.Group>
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>Reference Plan</Form.Label>
                                                <Form.Control type="text" id="WINSTORY_REFERENCE_PLAN" name='WINSTORY_REFERENCE_PLAN'
                                                    value={this.state.WINSTORY_REFERENCE_PLAN}
                                                    onChange={e => this.handleChange(e)} placeholder="Enter Reference Plan" /><br />
                                               
                                            </Form.Group>
                                        </Row>
                                        <Row>
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>Reference Assets</Form.Label>
                                                <Form.Control type="text" id="WINSTORY_REFERENCE_ASSET" name='WINSTORY_REFERENCE_ASSET'
                                                    value={this.state.WINSTORY_REFERENCE_ASSET}
                                                    onChange={e => this.handleChange(e)} placeholder="Enter Referene Asset"  /><br />
                                                
                                            </Form.Group>
                                            <Form.Group as={Col} md={6} >
                                                <Form.Label>Notes</Form.Label>
                                                <Form.Control type="text" id="WINSTORY_NOTES" name='WINSTORY_NOTES'
                                                    value={this.state.WINSTORY_NOTES}
                                                    onChange={e => this.handleChange(e)} placeholder="Enter Note" /><br />
