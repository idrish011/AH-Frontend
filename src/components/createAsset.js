...
//             <input type="file" name="uploadDocuments" id="documents" encType="multipart/form-data" className="documentUpload" onChange={props.onChange} />
...
//             <div style={{ visibility: this.state.id ? 'visible' : 'hidden' }}><img src="../img/clear.png" alt="" height="20px" onClick={props.onClick} /></div>
//         </Form.Group>
//     );
// }
...
class AssetDetails extends Component {
    ...
    componentDidMount() {
        ...
        if (url.indexOf('?') != -1) {
            if (url.indexOf('&governanceTab=') != -1) {
                var prevTab = url.split('&governanceTab=');
                ...
                .then(res => {
                    ...
                    for (var coltrl = 0; coltrl <= res.data.LINKS.length - 1; coltrl++) {
                        if (res.data.LINKS[coltrl].TYPE == "DOCUMENT") {
                            for (var k = 0; k <= res.data.LINKS[coltrl].arr.length - 1; k++) {
                                var parts = res.data.LINKS[coltrl].arr[k].LINK_URL.split('/');
                                var lastSegment = parts.pop() || parts.pop();
                                res.data.LINKS[coltrl].arr[k].file_name = lastSegment;
                                documenttemp.push(res.data.LINKS[coltrl].arr[k]);
                            }
                            this.setState({ documentLinks: documenttemp });
                        } else
                            if (res.data.LINKS[coltrl].TYPE == "MEDIA") {
                                this.setState({ MediaLinks: res.data.LINKS[coltrl].arr });
                            } else if (res.data.LINKS[coltrl].TYPE == "CODE REPOSITORY") {
                                this.setState({ CodeLinks: res.data.LINKS[coltrl].arr });
                            } else if (res.data.LINKS[coltrl].TYPE == "RELATED LINK") {
                                this.setState({ RelatedLinks: res.data.LINKS[coltrl].arr });
                            }
                    }
                    setTimeout(function () {
                        ...
                        var arrAna = []; var arrdb = []; var arrcloud = []; var arrsecurity = []; var arrappdev = []; var arrnatsaleplay = [];
                        for (var valuesol = 0; valuesol <= res.data.FILTERMAP.length - 1; valuesol++) {
                            ...
                            // }
                            // }
                            if (res.data.FILTERMAP[valuesol].TYPE == 'App Dev and Middleware') {
                                var selectedElement = document.getElementById('AppDev');

                                for (var p = 0; p <= res.data.FILTERMAP[valuesol].arr.length - 1; p++) {
                                    arrappdev.push(res.data.FILTERMAP[valuesol].arr[p].FILTER_ID);
                                }
                                var values = arrappdev;
                                for (var selectSol = 0; selectSol < selectedElement.options.length; selectSol++) {
                            ...
                            if (res.data.FILTERMAP[valuesol].TYPE == 'Database') {
                                var selectedElement = document.getElementById('Database');

                                for (var p = 0; p <= res.data.FILTERMAP[valuesol].arr.length - 1; p++) {
                                    arrdb.push(res.data.FILTERMAP[valuesol].arr[p].FILTER_ID);
                                }
                                var values = arrdb;
                                for (var selectSol = 0; selectSol < selectedElement.options.length; selectSol++) {
                                    selectedElement.options[selectSol].selected = values.indexOf(selectedElement.options[selectSol].value) >= 0;
                            ...
                            if (res.data.FILTERMAP[valuesol].TYPE == 'Cloud') {
                                var selectedElement = document.getElementById('Cloud');

                                for (var p = 0; p <= res.data.FILTERMAP[valuesol].arr.length - 1; p++) {
                                    arrcloud.push(res.data.FILTERMAP[valuesol].arr[p].FILTER_ID);
                                }
                                var values = arrcloud;
                                for (var selectSol = 0; selectSol < selectedElement.options.length; selectSol++) {
                                    //    console.log(selectedElement.options[selectSol].value);
                            ...
                            if (res.data.FILTERMAP[valuesol].TYPE == 'License FY21 Sales Plays') {
                                var selectedElement = document.getElementById('NatSalesPlay');

                                for (var p = 0; p <= res.data.FILTERMAP[valuesol].arr.length - 1; p++) {
                                    arrnatsaleplay.push(res.data.FILTERMAP[valuesol].arr[p].FILTER_ID);
                                }
                                var values = arrnatsaleplay;
                                for (var selectSol = 0; selectSol < selectedElement.options.length; selectSol++) {
                                    //    console.log(selectedElement.options[selectSol].value);
                                    selectedElement.options[selectSol].selected = values.indexOf(selectedElement.options[selectSol].value) >= 0;
                                    //    console.log(values.indexOf(selectedElement.options[selectSol].value) >= 0);
                            ...
                            if (res.data.FILTERMAP[valuesol].TYPE == 'Security') {
                                var selectedElement = document.getElementById('Security');

                                for (var p = 0; p <= res.data.FILTERMAP[valuesol].arr.length - 1; p++) {
                                    arrsecurity.push(res.data.FILTERMAP[valuesol].arr[p].FILTER_ID);
                                }
                                var values = arrsecurity;
                                for (var selectSol = 0; selectSol < selectedElement.options.length; selectSol++) {
                                    selectedElement.options[selectSol].selected = values.indexOf(selectedElement.options[selectSol].value) >= 0;
                                }
                            }
                            if (res.data.FILTERMAP[valuesol].TYPE == 'Analytics') {
                                var selectedElement = document.getElementById('Analytics');

                                for (var p = 0; p <= res.data.FILTERMAP[valuesol].arr.length - 1; p++) {
                                    arrAna.push(res.data.FILTERMAP[valuesol].arr[p].FILTER_ID);
                                }
                                var values = arrAna;
                                for (var selectSol = 0; selectSol < selectedElement.options.length; selectSol++) {
                                    //    console.log(selectedElement.options[selectSol].value);
                            ...
                            if (res.data.FILTERMAP[valuesol].TYPE == 'Cloud Services') {
                    ...
                    this.setState({
                        ...
                        ASSET_CREATEDBY: res.data.ASSET_CREATEDBY,
        ...
        axios.get(global.Ip + global.Port + '/asset/allfilters', {
            ...
            for (var i = 0; i < res.data.allFilters.length; i++) {
                ...
                //     })
                // }
                else if (FilterData[i].Type == "Cloud Services") {
                    //console.log(FilterData[i].Type);
                ...
                } else if (FilterData[i].Type == "Analytics") {
                    var IndustryAreaDrop = "";
                    for (var Industryfilter = 0; Industryfilter < FilterData[i].filters.length; Industryfilter++) {
                        IndustryAreaDrop += "<option value='" + FilterData[i].filters[Industryfilter].FILTER_ID + "'>" + FilterData[i].filters[Industryfilter].FILTER_NAME + '</option>';
                    }
                    this.setState({
                        AnalyticsFilterdrop: IndustryAreaDrop
                    })
                } else if (FilterData[i].Type == "Database") {
                    var DatebaseDrop = "";
                    for (var k = 0; k < FilterData[i].filters.length; k++) {
                        DatebaseDrop += "<option value='" + FilterData[i].filters[k].FILTER_ID + "'>" + FilterData[i].filters[k].FILTER_NAME + '</option>';
                    }
                    this.setState({
                        DatabaseFilterdrop: DatebaseDrop
                    })
                } else if (FilterData[i].Type == "App Dev and Middleware") {
                    var DatebaseDrop = "";
                    for (var k = 0; k < FilterData[i].filters.length; k++) {
                        DatebaseDrop += "<option value='" + FilterData[i].filters[k].FILTER_ID + "'>" + FilterData[i].filters[k].FILTER_NAME + '</option>';
                    }
                    this.setState({
                        AppDevFilterdrop: DatebaseDrop
                    })
                }
                else if (FilterData[i].Type == "Cloud") {
                    var DatebaseDrop = "";
                    for (var k = 0; k < FilterData[i].filters.length; k++) {
                        DatebaseDrop += "<option value='" + FilterData[i].filters[k].FILTER_ID + "'>" + FilterData[i].filters[k].FILTER_NAME + '</option>';
                    }
                    this.setState({
                        CloudFilterdrop: DatebaseDrop
                    })
                }
                else if (FilterData[i].Type == "Security") {
                    var DatebaseDrop = "";
                    for (var k = 0; k < FilterData[i].filters.length; k++) {
                        DatebaseDrop += "<option value='" + FilterData[i].filters[k].FILTER_ID + "'>" + FilterData[i].filters[k].FILTER_NAME + '</option>';
                    }
                    this.setState({
                        SecurityFilterdrop: DatebaseDrop
                    })
                } else if (FilterData[i].Type == "License FY21 Sales Plays") {
                    var DatebaseDrop = "";
                    for (var k = 0; k < FilterData[i].filters.length; k++) {
                        DatebaseDrop += "<option value='" + FilterData[i].filters[k].FILTER_ID + "'>" + FilterData[i].filters[k].FILTER_NAME + '</option>';
                    }
                    this.setState({
                        SalesPlayFilterdrop: DatebaseDrop
                    })
                }
            }
    ...
    deleteNode = (linkId, id) => {
        var r = window.confirm("Are you sure to delete this collateral?");
        if (r == true) {
            axios.delete(global.Ip + global.Port + '/asset/deletelinksbyid', {
                headers: {
                    "user_email": sessionStorage.getItem("user_email"),
                    "linkids": linkId
    ...
    onAddNew = (e) => {
        ...
        var cln = itm.cloneNode(true);
        cln.classList.remove("hiddenItem");
        ...
        if (e != "RelatedLink") {
            ...
            cln.children[2].children[0].innerHTML = '<img src="../img/clear.png" alt="" height="20px" onclick="document.getElementById(\'' + cln.id + '\').remove();" />';
        }
        //console.log(cln);
    ...
    onTempSave = (e) => {
        ...
        var dropDownAnalytics = document.getElementById('Analytics'), n;
        var dropDownDatabase = document.getElementById('Database'), db;
        var dropDownAppDev = document.getElementById('AppDev'), ad;
        var dropDownCloud = document.getElementById('Cloud'), cl;
        var dropDownSecurity = document.getElementById('Security'), s;
        var dropDownNatSalesPlay = document.getElementById('NatSalesPlay'), sp;
        // var dropDownAssetType = document.getElementById('AssetType'), t;
        var dropDownSalesPlay = document.getElementById('SalesPlay'), k;

        for (n = 0; n < dropDownAnalytics.options.length; n += 1) {
            if (dropDownAnalytics.options[n].selected) {
                //console.log('dropDownAnalytics: ' + dropDownAnalytics.options[n].value)
                SolutionAreaArray.push({ Value: dropDownAnalytics.options[n].value });
            }

        }
        for (sp = 0; sp < dropDownNatSalesPlay.options.length; sp += 1) {
            if (dropDownNatSalesPlay.options[sp].selected) {
                SolutionAreaArray.push({ Value: dropDownNatSalesPlay.options[sp].value });
            }
        }

        for (db = 0; db < dropDownDatabase.options.length; db += 1) {
            if (dropDownDatabase.options[db].selected) {
                //console.log('dropDownDatabase: ' + dropDownDatabase.options[n].value)
                SolutionAreaArray.push({ Value: dropDownDatabase.options[db].value });
            }

        }


        for (ad = 0; ad < dropDownAppDev.options.length; ad += 1) {
            if (dropDownAppDev.options[ad].selected) {
                SolutionAreaArray.push({ Value: dropDownAppDev.options[ad].value });
            }

        }
        for (cl = 0; cl < dropDownCloud.options.length; cl += 1) {
            if (dropDownCloud.options[cl].selected) {
                SolutionAreaArray.push({ Value: dropDownCloud.options[cl].value });
            }

        }
        for (s = 0; s < dropDownSecurity.options.length; s += 1) {
            if (dropDownSecurity.options[s].selected) {
                SolutionAreaArray.push({ Value: dropDownSecurity.options[s].value });
            }
        ...
        if (document.getElementById('AssetId').value == "") {
                                    ...
                                    axios.post(global.Ip + global.Port + '/asset/uploadImages/' + global.AssetIdCreated, global.thumbnail, {
                                        ...
                                        if (response.data == "working" || response.data == "done") {
                                            if (this.state.documentdata.length > 0) {
                                                ...
                                                alert('Your asset has been saved. Kindly check in "My Assets", for further updates to submit it for governance process');
                                                this.setState({
                                                    LoginSuccessRedirect: true
                                                });
                                            } else {
            ...
            var reqParmsEdit = {
                ...
                "userCase": this.state.mdeUseCaseValue,
                "customer": this.state.Customer,
                "createdBy": this.state.ASSET_CREATEDBY,
                "owner": this.state.AssetOwner,
                "scrmId": this.state.ScrmID,
                "oppId": null,
                'expiryDate': this.state.AssetExpiryDate,
    ...
    onCreateClick = (e) => {
        ...
        for (sp = 0; sp < dropDownNatSalesPlay.options.length; sp += 1) {
            //console.log('dropDownCloud.options.length: ' + dropDownNatSalesPlay.options.length)
            if (dropDownNatSalesPlay.options[sp].selected) {
                SolutionAreaArray.push({ Value: dropDownNatSalesPlay.options[sp].value });
            }
        }
        ...
        if (!this.state.id) {
            if (document.getElementById('PresentationLink').children[1].children[0].value != "" && document.getElementById('PresentationLink').children[0].children[0].value != "") {
                ...
                documentUploadHeader.push(headerObj);
                //.log(document.getElementById('PresentationLink').children[0].children[0].value);
                //console.log(document.getElementById('PresentationLink').children[2].children[0].value);
            } else if (document.getElementById('PresentationLink').children[0].children[0].value == "" && (document.getElementById('PresentationLink').children[1].children[0].value != "" || document.getElementById('PresentationLink').children[2].children[0].value != "")) {
                // if (!document.getElementById('PresentationLink').children[1].children[1].children[1].innerHTML) {
                alert('Please fill Document name');
        ...
        // }
        if (document.getElementById('AssetId').value == "") {
            //  alert(document.getElementById('upload-cover-photo').value+global.CoverPhoto);
                                    ...
                                    axios.post(global.Ip + global.Port + '/asset/uploadImages/' + global.AssetIdCreated, global.thumbnail, {
                                        ...
                                        if (response.data == "working" || response.data == "done") {
                                            if (this.state.documentdata.length > 0) {
                                                ...
                                                alert('Your asset has been submitted. Kindly check in "My Assets", for further updates as part of governance process');
                                                this.setState({
                                                    LoginSuccessRedirect: true
                                                });
                                            } else {
    ...
    render() {
        ...
        if (this.state.SuccessEditRedirect) {
            window.history.go(-1);
        }
        var visibleClass = this.state.id ? 'hiddenItem' : '';
        return (
            <div className="App">
                <Header />
                <div className="BodyContainer mt-4">
                    <Container fluid={false}>
                        ...
                        <Row>
                            <Col md={12}>
                                <p className="mt-4">
                                    <small>
                                        {this.state.AssetFormSelectionLabel == "Solution Asset" &&
                                            <>
                                                Solution Asset is an asset that demonstrates a working use case with solution architecture, may or may not have a demo video, and may or may not have demo collateral. <br />
                                    Please note that it is strongly recommended to have demo video and collateral so that sales teams can reduce the cycles by using it as is with their customers to start the conversation.
                                    </>
                                        }
                                        {this.state.AssetFormSelectionLabel == "White Paper" &&
                                            <>
                                                White Paper is a technical document that describes how a technology or product solves a particular problem. It's a marketing document and a technical document, that is meant to help readers understand an issue, solve a problem, or make a decision.
                                    </>
                                        }
                                        {this.state.AssetFormSelectionLabel == "Workshop" &&
                                            <>
                                                Workshop is a set of Hands-on-Labs that demonstrate a step-by-step guide to execute specific technical concept(s) like usage of a product/service, building a solution or building sales play etc. A Workshop needs to have a step-by-step guide as a document or git lab, may or may not have a workshop video, may or may not have notional architecture.
                                    </>
                                        }
                                    </small>
                  ...
                  <Form.Label>Location</Form.Label>
                    ...
                    onChange={e => this.handleChange(e)} placeholder="Enter Location" />
                      ...
                      <a href={this.state.ASSET_Video} target="_blank" class={this.state.class}>View</a>}
                                    ...
                                    <hr />
                                    <div className="title flex" onClick={this.expandClick.bind(this, "extraFilter")}>
                                        <div class="nowrap">NAT Filters </div>

                                        <input type="hidden" id="extraFilterval" value="true" />
                                        <div class="titleLine"></div>
                                        <span style={{float:'right'}} className="expand expandColor" style={{ marginRight: 0 }} id="extraFilterexpand">+</span>
                                        
                                    </div>
                                    {/* <Row>
                                        <Form.Group as={Col} md={12} >
                                            <h6 style={{ borderBottom: 1 + 'px solid', paddingBottom: 10, margin: 0, fontWeight: 'bold', fontSize: '14px' }} onClick={this.expandClick.bind(this, "extraFilter")}>NAT Filters <span className="expand" style={{ marginRight: 0 }} id="extraFilterexpand">+</span></h6>
                                            <input type="hidden" id="extraFilterval" value="true" />
                                        </Form.Group>

                                    </Row> */}
                                    <Row id="extraFilter" className="hide" style={{ paddingLeft: 20, paddingRight: 20, }}>

                                        <Row>
                                            <Form.Group as={Col} md={6}>
                                                <Form.Label>Analytics <span title="All Analytics this asset directly addresses should be selected">&#x1f6c8;</span></Form.Label>
                                                <Form.Control as="select" multiple id="Analytics" name='Analytics' onChange={e => this.handleChange(e)} dangerouslySetInnerHTML={{ __html: this.state.AnalyticsFilterdrop }}>
                                                </Form.Control>
                                            </Form.Group>
                                            <Form.Group as={Col} md={6}>
                                                <Form.Label>Database <span title="All Database consumers that should be targeted with this asset should be selected">&#x1f6c8;</span></Form.Label>
                                                <Form.Control as="select" multiple id="Database" name='Database' onChange={e => this.handleChange(e)} dangerouslySetInnerHTML={{ __html: this.state.DatabaseFilterdrop }}>
                                                </Form.Control>
                                            </Form.Group>
                                            <Form.Group as={Col} md={6}>
                                                <Form.Label>App Dev and Middleware <span title="All App Dev and Middleware this asset directly addresses should be selected">&#x1f6c8;</span></Form.Label>
                                                <Form.Control as="select" multiple id="AppDev" name='AppDev' onChange={e => this.handleChange(e)} dangerouslySetInnerHTML={{ __html: this.state.AppDevFilterdrop }}>
                                                </Form.Control>
                                            </Form.Group>
                                            <Form.Group as={Col} md={6}>
                                                <Form.Label>Cloud <span title="All Cloud consumers that should be targeted with this asset should be selected">&#x1f6c8;</span></Form.Label>
                                                <Form.Control as="select" multiple id="Cloud" name='Cloud' onChange={e => this.handleChange(e)} dangerouslySetInnerHTML={{ __html: this.state.CloudFilterdrop }}>
                                                </Form.Control>
                                            </Form.Group>
                                            <Form.Group as={Col} md={6}>
                                                <Form.Label>Security <span title="All Security this asset directly addresses should be selected">&#x1f6c8;</span></Form.Label>
                                                <Form.Control as="select" multiple id="Security" name='Security' onChange={e => this.handleChange(e)} dangerouslySetInnerHTML={{ __html: this.state.SecurityFilterdrop }}>
                                                </Form.Control>
                                            </Form.Group>
                                            <Form.Group as={Col} md={6}>
                                                <Form.Label>License FY21 Sales Plays <span title="All License FY21 Sales Plays this asset directly addresses should be selected">&#x1f6c8;</span></Form.Label>
                                                <Form.Control as="select" multiple id="NatSalesPlay" name='SalesPlay' onChange={e => this.handleChange(e)} dangerouslySetInnerHTML={{ __html: this.state.SalesPlayFilterdrop }}>
                                                </Form.Control>
                                            </Form.Group>
                                        </Row>
                                    </Row>
                  ...
                  <Form.Group as={Col}  md={6} controlId="formGridEmail">
                                    ...
                                    <div>
                                        ...
                                        {this.state.documentLinks.map((item, index) => (
                                            <Row id={'PresentationLink' + index}>
                                                <Form.Group as={Col} md={3} >
                                                    <Form.Control type="text" disabled="true" maxLength="255" id="documentName" value={item.LINK_DESCRIPTION} placeholder="Enter Document Name" />
                                                </Form.Group>
                                                <Form.Group as={Col} md={5} >
                                                    {/* <input type="file" disabled="true" name="uploadDocuments" id="documents" encType="multipart/form-data" className="documentUpload" onChange={e => this.UploadMediaFile(e)} /> */}
                                                    <div class={this.state.class}>
                                                        <img src="../img/Reports.png" width="20px" height="20px" />&nbsp;
                                                ...
                                                <Form.Group as={Col} md={3} > <Form.Control type="textarea" disabled="true" maxLength="4000" value={item.LINK_DESCRIPTION_DATA} placeholder="Enter Description" />
                                                </Form.Group>
                                                <Form.Group as={Col} md={1} >
                                                    <div style={{ visibility: this.state.id ? 'visible' : 'hidden' }}><img src="../img/clear.png" alt="" height="20px" onClick={(e) => this.deleteNode(item.LINK_ID, 'PresentationLink' + index)} /></div>
                                                </Form.Group> </Row>
                                        ))}
                                        <Row id="PresentationLink" className={visibleClass}>
                                            <Form.Group as={Col} md={3} >
                                                <Form.Control type="text" maxLength="255" id="documentName" placeholder="Enter Document Name" />
                                            </Form.Group>
                                            <Form.Group as="file" as={Col} md={3} >
                                    ...
                                    <div>
                                        ...
                                        <div id="MediaLinkAdd">
                                            {this.state.MediaLinks.map((item, index) => (
                                                <Row id={'MediaLinkAdd' + index}>
                                                    <Form.Group as={Col} md={3} >
                                                        <Form.Control type="text" maxLength="255" placeholder="Enter Media Name" value={item.LINK_DESCRIPTION} />
                                                    </Form.Group>
                                                    <Form.Group as={Col} md={3} >
                                                        <Form.Control type="text" maxLength="2000" placeholder="Enter Media Link" value={item.LINK_URL} />
                                                    </Form.Group>
                                                    <Form.Group as={Col} md={5} >
                                                        <Form.Control type="textarea" maxLength="4000" placeholder="Enter Description" value={item.LINK_DESCRIPTION_DATA} />
                                                    </Form.Group>
                                                    <Form.Group as={Col} md={1} >
                                                        <div style={{ visibility: this.state.id ? 'visible' : 'hidden' }}><img src="../img/clear.png" alt="" height="20px" onClick={(e) => this.deleteNode(item.LINK_ID, 'MediaLinkAdd' + index)} /></div>
                                                    </Form.Group>
                                                </Row>
                                            ))}
                                        </div>
                                        <Row id="MediaLink" className={visibleClass}>
                                            <Form.Group as={Col} md={3} >
                                    ...
                                    <div>
                                        ...
                                        <div id="CodeLinkAdd">
                                            {this.state.CodeLinks.map((item, index) => (
                                                <Row id={'CodeLinkAdd' + index}>
                                                    <Form.Group as={Col} md={3} >
                                                        <Form.Control type="text" maxLength="255" placeholder="Enter Code Repo Name" value={item.LINK_DESCRIPTION} />
                                                    </Form.Group>
                                                    <Form.Group as={Col} md={3} >
                                                        <Form.Control type="text" maxLength="2000" placeholder="Enter Code Repo Link" value={item.LINK_URL} />
                                                    </Form.Group>
                                                    <Form.Group as={Col} md={5} >
                                                        <Form.Control type="textarea" maxLength="4000" placeholder="Enter Description" value={item.LINK_DESCRIPTION_DATA} />
                                                    </Form.Group>
                                                    <Form.Group as={Col} md={1} >
                                                        <div style={{ visibility: this.state.id ? 'visible' : 'hidden' }}><img src="../img/clear.png" alt="" height="20px" onClick={(e) => this.deleteNode(item.LINK_ID, 'CodeLinkAdd' + index)} /></div>
                                                    </Form.Group>
                                                </Row>
                                            ))}
                                        </div>
                                        <Row id="CodeLink" className={visibleClass}>
                                            <Form.Group as={Col} md={3} >
                                                {/* <Form.Label>Enter Media File Name</Form.Label> */}
                                    ...
                                    <div>
                                        ...
                                        <div id="RelatedLinkAdd">
                                            {this.state.RelatedLinks.map((item, index) => (
                                                <Row id={'RelatedLinkAdd' + index}>
                                                    <Form.Group as={Col} md={5} >
                                                        <Form.Control type="text" maxLength="2000" placeholder="Enter Link" value={item.LINK_URL} />
                                                    </Form.Group>
                                                    <Form.Group as={Col} md={6} >
                                                        <Form.Control type="text" maxLength="4000" placeholder="Enter Description" value={item.LINK_DESCRIPTION_DATA} />
                                                    </Form.Group>
                                                    <Form.Group as={Col} md={1} >
                                                        <div style={{ visibility: this.state.id ? 'visible' : 'hidden' }}><img src="../img/clear.png" alt="" height="20px" onClick={(e) => this.deleteNode(item.LINK_ID, 'RelatedLinkAdd' + index)} /></div>
                                                    </Form.Group>
                                                </Row>
                                            ))}
                                        </div>
                                        <Row id="RelatedLink" className={visibleClass}>
                                            <Form.Group as={Col} md={5} >
                                    ...
                                    {/* <Form.Group as={Col} md={2} >
                                        ...
                                        <a variant="primary" className="text-center mb20 CreateAsset" onClick={(e) => this.onTempSave(e)} type="submit">
                                           SAVE
                                        </a>
                                    ...
                                    <div className="FooterBtn">
                                        <input type="hidden" id="AssetId" name="AssetId" value={this.state.id} />
                                        {(this.state.prevTabGovernance != 'Y') && <a variant="primary" className="text-center mb20 CreateAsset" onClick={(e) => this.onTempSave(e)} type="submit">
                                            SAVE
                                        </a>}
                                        <a variant="primary" className="text-center mb20 CreateAsset" onClick={(e) => this.onCreateClick(e)} type="submit">
                                            {this.state.msgbtn}
